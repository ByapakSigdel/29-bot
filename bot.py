from utils import get_suit, get_suit_cards, get_partner_idx, pick_winning_card_idx, is_high, index, find, get_card_info


def get_bid(body):

    cards = body.get("cards")
    trump_suit = body.get("trumpSuit")
   
    if len(body["bidHistory"])!=0:
     points=0
     for card in cards:
        points += get_card_info(card)["points"]
     ...
     bid = body.get("bidHistory")

     print(bid[1][1])
     biddy=16
     if bid[1][1]>biddy:
         
         if points >= 6 or points<=9:
            biddy=biddy+1
            return {"bid": biddy}
     else:
         biddy=biddy+1
